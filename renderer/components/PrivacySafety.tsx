...
import { Shield, AlertTriangle, CheckCircle, Eye, TrendingUp, Users, Flag } from 'lucide-react';
...
export default function PrivacySafety() {
  ...
  const [activeTab, setActiveTab] = useState<'analyze' | 'education' | 'report' | 'stats'>('analyze');
  ...
  const analyzeConversation = async () => {
    ...
    try {
      ...
      const result = await window.electronAPI.analyzeConversationSafety(messages);
  ...
  const quickSafetyCheck = async () => {
    ...
    try {
      const result = await window.electronAPI.quickSafetyCheck(quickMessage.trim());
  ...
  return (
    <div style={{ padding: '20px', color: '#ffffff', maxHeight: '80vh', overflow: 'auto' }}>
      <div style={{ 
        display: 'flex', 
        alignItems: 'center', 
        marginBottom: '20px' 
      ...
      <div style={{ 
        ...
        {[
          { id: 'analyze', label: '🛡️ Analyze', icon: Shield },
          ...
          <button
            ...
            style={{
              background: activeTab === tab.id ? 'rgba(59, 130, 246, 0.8)' : 'rgba(255, 255, 255, 0.1)',
        ...
        <div>
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
              ...
              <button
                ...
                style={{
                  background: quickMessage.trim() ? 'rgba(59, 130, 246, 0.8)' : 'rgba(255, 255, 255, 0.1)',
                  border: '1px solid rgba(255, 255, 255, 0.3)',
                  color: 'white',
                  borderRadius: '4px',
                  padding: '8px 16px',
            ...
            {quickCheckResult && (
              <div style={{
                background: quickCheckResult.hasRisk ? 'rgba(239, 68, 68, 0.1)' : 'rgba(34, 197, 94, 0.1)',
                border: `1px solid ${quickCheckResult.hasRisk ? 'rgba(239, 68, 68, 0.3)' : 'rgba(34, 197, 94, 0.3)'}`,
                borderRadius: '6px',
                padding: '12px',
                fontSize: '13px'
              }}>
                <div style={{ fontWeight: '600', marginBottom: '6px' }}>
                  {quickCheckResult.hasRisk ? '⚠️' : '✅'} Risk Level: {quickCheckResult.riskLevel.toUpperCase()}
                </div>
                {quickCheckResult.alerts.map((alert: string, index: number) => (
                  <div key={index} style={{ marginBottom: '4px' }}>{alert}</div>
                ))}
              </div>
            )}
          </div>
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              Paste your conversation below. Format: "Contact: message" or "User: message" (one per line)
            </div>
...
Contact: Great! Want to text instead? Here's my number: 555-0123`}
            ...
            <button
              ...
              style={{
                background: inputMessages.trim() ? 'rgba(59, 130, 246, 0.8)' : 'rgba(255, 255, 255, 0.1)',
                border: '1px solid rgba(255, 255, 255, 0.3)',
                color: 'white',
                borderRadius: '6px',
                padding: '10px 20px',
            ...
            >
              {loading ? '🔄 Analyzing...' : '🛡️ Analyze Safety'}
            </button>
          </div>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    height: '60px',
                    borderRadius: '50%',
                    background: `conic-gradient(${getRiskLevelText(analysisResult.combinedRisk).color} ${analysisResult.combinedRisk * 360}deg, rgba(255,255,255,0.1) 0deg)`,
                    display: 'flex',
                    alignItems: 'center',
                  ...
                  <div>
                    <div style={{ 
                      fontSize: '16px', 
                      fontWeight: '600',
                      color: getRiskLevelText(analysisResult.combinedRisk).color
                    }}>
              ...
              {analysisResult.patternAnalysis.alerts.length > 0 && (
                <div style={{ marginBottom: '20px' }}>
                  <div style={{ 
                    ...
                    🚨 Security Alerts ({analysisResult.patternAnalysis.alerts.length})
                  </div>
                  {analysisResult.patternAnalysis.alerts.map((alert, index) => (
                    ...
                    >
                      <div style={{ 
                        ...
                        <span style={{ fontSize: '16px' }}>{getSeverityIcon(alert.severity)}</span>
                        <span style={{ 
                          fontWeight: '600', 
              ...
              {analysisResult.aiAnalysis && (
                <div style={{ marginBottom: '20px' }}>
                  <div style={{ 
                    ...
                    🤖 AI Analysis (Trust Score: {analysisResult.aiAnalysis.trustScore}%)
                  </div>
                  ...
                  {analysisResult.aiAnalysis.redFlags.length > 0 && (
                    <div style={{ marginBottom: '12px' }}>
                      ...
                      {analysisResult.aiAnalysis.redFlags.map((flag, index) => (
                        <div key={index} style={{ fontSize: '12px', opacity: 0.9, marginLeft: '16px' }}>
                          • {flag}
                        </div>
                      ))}
                    </div>
                  ...
                  {analysisResult.aiAnalysis.positiveSignals.length > 0 && (
                    <div style={{ marginBottom: '12px' }}>
                      ...
                      {analysisResult.aiAnalysis.positiveSignals.map((signal, index) => (
                        <div key={index} style={{ fontSize: '12px', opacity: 0.9, marginLeft: '16px' }}>
                          • {signal}
                        </div>
                      ))}
        ...
        <div>
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ 
              ...
              {safetyEducation.scamWarnings.map((warning: string, index: number) => (
                <div key={index} style={{ fontSize: '12px', marginBottom: '4px' }}>
                  • {warning}
                </div>
              ))}
      ...
      {activeTab === 'report' && (
        <div>
          ...
          <div style={{ 
            ...
            <div style={{ marginBottom: '12px' }}>
              Report suspicious behavior, scams, or safety concerns to help protect the community.
            </div>
            <div style={{ 
              fontSize: '11px', 
              opacity: 0.7 
            }}>
              This feature would integrate with the dating app's reporting system in a production environment.
            </div>
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
            <div style={{ 
              ...
              <div style={{ fontSize: '11px', opacity: 0.8 }}>Total Analyses</div>
            </div>
            <div style={{ 
              background: 'rgba(255, 255, 255, 0.1)', 
              borderRadius: '8px', 
              ...
              <div style={{ fontSize: '11px', opacity: 0.8 }}>Risks Detected</div>
            </div>
          </div>
          ...
          <div style={{ 
            ...
            <div style={{
              ...
              <div style={{
                background: safetyStats.safetyScore > 80 ? '#22c55e' : safetyStats.safetyScore > 60 ? '#f59e0b' : '#ef4444',
                height: '100%',

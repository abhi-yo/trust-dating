...
import { Key, TestTube, Info, Keyboard } from 'lucide-react';
    ...
    const loadSettings = async () => {
      try {
        ...
        try {
          ...
          const models = await window.electronAPI.getAvailableModels(config.provider);
  ...
  const handleOpacityChange = async (newOpacity: number) => {
    ...
    try {
      ...
      await window.electronAPI.setApiKey(
        currentProvider as 'gemini' | 'openai' | 'anthropic' | 'openrouter' | 'custom', 
        apiKey,
        options
      );
    ...
    <div style={{ 
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', fontWeight: '500', marginBottom: '8px', opacity: 0.9 }}>
            Daily API Usage
          </div>
          ...
          <div style={{
            ...
            <div style={{
              width: `${Math.min(100, (apiUsage.dailyUsed / apiUsage.dailyLimit) * 100)}%`,
              height: '100%',
          ...
          <div style={{ 
            ...
            Cache: {apiUsage.cacheSize} responses • Total: {apiUsage.totalCalls} calls
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', fontWeight: '500', marginBottom: '12px', opacity: 0.9 }}>
            AI Provider
          </div>
          
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '12px' }}>
            {['gemini', 'openrouter', 'openai', 'anthropic', 'custom'].map(provider => (
              <button
                key={provider}
                onClick={() => handleProviderChange(provider)}
                style={{
                  background: currentProvider === provider ? 'rgba(59, 130, 246, 0.3)' : 'rgba(255, 255, 255, 0.1)',
                  border: currentProvider === provider ? '1px solid #3b82f6' : '1px solid rgba(255, 255, 255, 0.2)',
                  borderRadius: '6px',
                  padding: '8px',
                  fontSize: '11px',
                  color: '#fff',
          ...
          {(currentProvider === 'openrouter' || currentProvider === 'custom') && (
            <div style={{ marginBottom: '12px' }}>
              ...
              {currentProvider === 'openrouter' ? (
                ...
                >
                  {availableModels.map(model => (
                    <option key={model} value={model} style={{ background: '#1f2937', color: '#fff' }}>
                      {model}
                    </option>
                  ))}
                </select>
          ...
          <div style={{ fontSize: '10px', opacity: 0.6, textAlign: 'center' }}>
            {currentProvider === 'openrouter' && 'Free Horizon Alpha model available'}
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', opacity: 0.8, marginBottom: '8px' }}>
            Current provider: <strong style={{ textTransform: 'capitalize' }}>{currentProvider}</strong>
          </div>
          ...
          {(showApiKeyInput || !hasApiKey) && (
            <div style={{ marginBottom: '12px' }}>
              <div style={{ fontSize: '12px', opacity: 0.8, marginBottom: '4px' }}>
                {hasApiKey ? 'Enter new API key:' : 'Enter your Gemini API key:'}
              </div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{
                    background: isUpdatingKey ? 'rgba(255, 255, 255, 0.1)' : 'rgba(255, 255, 255, 0.9)',
          ...
          {!hasApiKey && (
            <div style={{
              ...
              ⚠️ API key required for all analysis features to work
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontWeight: '500', marginBottom: '4px' }}>💡 When to update your API key:</div>
          <div style={{ opacity: 0.8, lineHeight: '1.4' }}>
            • When you see "API quota exceeded" errors<br/>
            • When responses stop working suddenly<br/>
            • When switching to a different Google account<br/>
            • For better rate limits or billing management
          </div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '8px' }}>
            <strong>Cmd/Ctrl + Shift + C</strong> - Force check clipboard & get replies
          </div>
